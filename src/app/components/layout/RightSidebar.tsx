 ...
 * Displays popular communities and wiki articles
 */
  ...
  return (
    <aside className="hidden lg:block lg:w-68 xl:w-72 sticky top-[48px] h-[calc(100vh-48px)] overflow-y-auto scrollbar-hide">
      <div className="p-3 lg:p-4 space-y-4">
        ...
        <section className="bg-card border border-border rounded-lg p-3 lg:p-4">
          <h2 className="text-foreground font-semibold mb-4">Topluluklar</h2>
          <div className="space-y-3">
            {communities.slice(0, 5).map((community) => (
              <button
                key={community.id}
                onClick={() => navigate(ROUTES.COMMUNITY_DETAIL(community.id))}
        ...
        <section className="bg-card border border-border rounded-lg p-3 lg:p-4">
          ...
          <div className="space-y-3">
            {wikiArticles.slice(0, 5).map((article) => (
              <button
                key={article.id}
                onClick={() => navigate(ROUTES.WIKI_DETAIL(article.id))}
              ...
              >
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-muted-foreground text-[10px] bg-accent px-1.5 py-0.5 rounded">
                    {article.category}
                  </span>
